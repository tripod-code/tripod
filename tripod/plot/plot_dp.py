...
def panel_new_dp(data, filename="data", extension="hdf5", im=0, ir=0, it=0, show_limits=True, show_St1=True):
...
def ipanel_new_dp(data, filename="data", extension="hdf5", im=0, ir=0, it=0, show_limits=True, show_St1=True):
    ...
    with out2:
        def plot2(it, ir, im):
            ...
            ax01.loglog(data.m[it, ...], data.sigmaDust[it, ir, :], c="C3")
            ax01.axvline(data.m[it, im], color="#AAAAAA", lw=1, ls="--")
            ax01.set_xlim(data.m[it, 0], data.m[it, -1])
